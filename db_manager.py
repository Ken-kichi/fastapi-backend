...
class DatabaseManager:
    ...
    def create_task_record(self,conn,title,description,user_id)->Message:
        created_time = datetime.now(pytz.timezone('Asia/Tokyo')).strftime("%Y-%m-%d %H:%M:%S.%f")
        cursor = conn.cursor()
        cursor.execute("""
                       INSERT INTO tasks (
                        user_id,
    ...
    def update_task_record(self,conn,id,title,description,completed,user_id)->Message:
        update_time = datetime.now(pytz.timezone('Asia/Tokyo')).strftime("%Y-%m-%d %H:%M:%S.%f")
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE tasks SET title = %s,description=%s,completed=%s,updated_at = %s ,user_id = %s WHERE id = %s",
            (title,description,completed,update_time,user_id,id)
        )
        conn.commit()
    ...
    def read_all_user_record(self,conn)->Union[UserRead,bool]:
        cursor = conn.cursor()
    ...
    def read_user_record_by_id(self,conn,user_id)->Union[UserReadAll,bool]:
        cursor = conn.cursor()
    ...
    def update_user_record(self,conn,user_id,username,full_name,email,is_manager,disabled):
        updated_time = datetime.now(pytz.timezone('Asia/Tokyo')).strftime("%Y-%m-%d %H:%M:%S.%f")
        cursor = conn.cursor()
        cursor.execute(
